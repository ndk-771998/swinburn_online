...
class AnalyticsQuerySeeder extends Seeder
{
    ...
    public function run()
    {
        AnalyticsQuery::insert([
            ["type"=>"widget","date_type"=>"","slug" => "monthly_revenue","query" => "SELECT SUM(total) AS value from orders where month(created_at) = month(CURRENT_DATE())"],
            ["type"=>"widget","date_type"=>"","slug" => "daily_revenue","query" => "SELECT SUM(total) AS value from orders where day(created_at) = day(CURRENT_DATE())"],
            ["type"=>"widget","date_type"=>"","slug" => "monthly_visited","query" => "SELECT 100 AS value"],
            ["type"=>"widget","date_type"=>"","slug" => "total_visited","query" => "SELECT 100 AS value"],
